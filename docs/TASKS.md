...
| ROOM-007 | ACTIVE | Govern Room Agent collaboration policy | ROOM-004, RUN-006, DISC-007, WEB-032 | Owner-configured Room policy is persisted and enforced server-side for Discussions, `@all`, Agent handoffs, bounded handoff depth, and one-shot multi-Agent routing; Room settings expose the policy alongside per-Agent participation with focused Server/Web regressions |
