...
| ID | State | Task | Depends On | Completion Evidence |
| --- | --- | --- | --- | --- |
| WSP-001 | DONE | Add opaque Workspace identity and source-read leases | GOV-009, BRG-004, TASK-001 | migration 0035, generated contract fields, service regressions, and Go Workspace/connection tests prove a Run/Task/Agent/Device/generation-bound `read_source` lease, exact retry, expiry/release, stale and terminal rejection, path-free publication, and old-Bridge compatibility |
| ART-001 | DONE | Persist durable Blob upload and seal | WSP-001, DATA-006 | migration 0036, local BlobStore and service regressions prove ordered idempotent chunks, fsync plus atomic content-addressed seal, exact status recovery, Team-scoped dedupe without upload bypass, active-upload quota, expiry, digest and symlink rejection, immutable metadata, and sealed-but-unbound restart recovery |
...
| BRG-028 | DONE | Publish one locally authorized Artifact snapshot | WSP-001, ART-001, TASK-010, CON-010 | `artifact publish`, Device-authenticated HTTP routes, source and response-loss Go regressions, Server route tests, and real Server/Bridge E2E prove Workspace containment and generation stability, bounded typed capture, resumable prepare/chunk/seal/bind, canonical revision visibility, exact retry recovery, capability publication, and no absolute path, storage key, or secret crosses the boundary |
...
| QA-020 | DONE | Verify Artifact-to-Artifact two-Bridge recovery | BRG-028, BRG-029, ADP-014, TASK-011, WEB-040, QA-004, QA-006 | `docs/acceptance/qa-020-artifact-handoff-recovery.md`, focused Connection/Delivery regressions, and deterministic real-Server/two-Bridge E2E prove A publishes Artifact A, B consumes identical staged bytes and publishes linked Artifact B across duplicate, response-loss, restart, digest-failure, terminal-replay, and offline cuts; physical-machine proof remains a separate extension of QA-002 |
