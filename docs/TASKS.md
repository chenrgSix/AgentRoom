...
| BRG-019 | READY | Version Bridge Runtime presets and add local self-test | BRG-018, BRG-007 | legacy Codex/Pi presets migrate deterministically without losing owner fields; the Console can run an explicit bounded read-only probe and render only safe success or failure diagnostics |
...
| WEB-021 | ACTIVE | Make Room timeline tail-aware and incrementally synchronized | WEB-020, ROOM-002 | initial load shows the newest 100 messages, cursor refresh appends messages beyond 100 exactly once, and a single-flight refresh cannot let stale responses overwrite newer state |
| WEB-022 | READY | Show safe Runtime failure diagnostics in Room | WEB-021, BRG-018 | failed Run cards load the authorized terminal event on demand and show localized error code, category, exit code, and actionable guidance without raw stderr or unknown detail keys |
