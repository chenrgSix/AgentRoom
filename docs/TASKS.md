...
| ROOM-007 | DONE | Govern Room Agent collaboration policy | ROOM-004, RUN-006, DISC-007, WEB-032 | migration 0020, 99 Server regressions, 19 Web regressions, the multi-target outbox test, 122 workspace tests, deterministic Bridge/MCP E2E, production builds, 7 schemas with 32 fixtures, docs lint, and isolated real-browser checks prove atomic Owner settings plus Server enforcement for Discussions, exact `@all`, exact Agent reply handoffs, depth 1-4, participant access, and one-shot multi-Agent routing with zero-overflow light, 760 px, and 390 px layouts |
