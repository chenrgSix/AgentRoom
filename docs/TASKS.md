...
| BRG-025 | ACTIVE | Pass a configured central Server Token from Bridge | BRG-024, SEC-001 | Server and Go negative tests prove an optional configured central Token gates Bridge join, claim, legacy pair, and WebSocket requests; Console edits address and Token without exposing it in state, logs, diagnostics, or existing credentials, while token-free local deployments remain compatible |
