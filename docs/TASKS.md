...
| CON-003 | READY | Define Bridge message schemas | CON-002 | hello, heartbeat, publish, and run schemas validate |
| CON-004 | PLANNED | Generate TypeScript and Go contract types | CON-003 | generation is deterministic and both languages agree on fixtures |
...
| REG-001 | PLANNED | Implement Member and Device registry | DATA-002, SEC-001 | membership and device ownership persist |
| REG-002 | PLANNED | Implement Agent publication and capability validation | REG-001, CON-003 | managed/manual agents publish correctly |
| REG-003 | PLANNED | Implement Presence TTL and derived status | REG-002 | ready, busy, degraded, manual, offline verified |
| WEB-001 | PLANNED | Scaffold Web shell and basic Team management | ROOM-001, REG-001 | user creates Team, Room, Member, and fake Agent |

## Workstream F2: Bridge and Managed Runtime

| ID | State | Task | Depends On | Completion Evidence |
| --- | --- | --- | --- | --- |
| BRG-001 | PLANNED | Scaffold Go Bridge process and configuration | CON-004, SEC-001 | binary starts with validated config |
| BRG-002 | PLANNED | Implement invitation pairing and credential storage | BRG-001, REG-001 | one-time invitation cannot be replayed |
...
| ADP-005 | PLANNED | Implement Generic CLI fallback | ADP-002 | stdout, exit, timeout, and cancel verified |

## Workstream F3: Run Orchestration and Web Experience

| ID | State | Task | Depends On | Completion Evidence |
| --- | --- | --- | --- | --- |
| RUN-001 | PLANNED | Create Run from a valid Mention | ROOM-003, REG-003 | one message creates the intended target Run |
...
| WEB-002 | PLANNED | Implement Team Room message timeline | WEB-001, ROOM-002 | reload preserves ordered history |
| WEB-003 | PLANNED | Implement structured Mention composer | WEB-002, ROOM-003 | display labels resolve to stable Agent IDs |
| WEB-004 | PLANNED | Implement Agent presence panel | WEB-001, REG-003 | all integration modes render accurately |
| WEB-005 | PLANNED | Implement Run card and live updates | WEB-002, RUN-003 | queued through terminal states are visible |
...
| ID | State | Task | Depends On | Completion Evidence |
| --- | --- | --- | --- | --- |
| DATA-003 | PLANNED | Persist Run, delivery, and event sequence state | RUN-003 | restart resumes without state regression |
| DATA-004 | PLANNED | Add backup, restore, and migration rollback procedure | DATA-003 | tested backup restores acceptance fixture |
| SEC-002 | PLANNED | Enforce Team, Room, Agent, and Run authorization | ROOM-003, RUN-002 | cross-Team and cross-Owner negative tests pass |
| SEC-003 | PLANNED | Add credential rotation and Device revoke propagation | BRG-003, SEC-002 | revoked Bridge cannot reconnect or receive Runs |
