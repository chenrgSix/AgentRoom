...
| QA-021 | ACTIVE | Publish and verify v0.2.0 | QA-019, QA-020, QA-008 | exact tagged source must pass main CI; stable and prerelease version strings must be verified without accepting a longer version suffix; the no-clobber workflow must build and verify five CLI archives, two macOS desktop archives, checksums, and license assets; the public stable Release must pass an independent 11-asset clean-download verification; `BRG-013` and `QA-002` remain explicit post-release physical-environment evidence rather than stable-version blockers |
