...
| CON-010 | DONE | Define Artifact content delivery and materialization receipts | TASK-010, CON-009 | 53 positive/negative fixtures, deterministic checked-in TypeScript/Go generation, both language validators, strict type checking, Server capability validation, and Bridge compilation carry content identity, bounded size, exact media type, digest, path-free logical alias, optional publication/materialization capabilities, and closed verified/reused receipt state without local paths or provider-native data |
