...
| ID | State | Task | Depends On | Completion Evidence |
| --- | --- | --- | --- | --- |
| WSP-001 | IN_PROGRESS | Add opaque Workspace identity and source-read leases | GOV-009, BRG-004, TASK-001 | regression repair is restoring lease-before-read ordering, live Run/Task/Agent invalidation, renewable attempt identity, and connection-local Workspace generation refresh before the original migration, contract, compatibility, and recovery evidence is accepted again |
| ART-001 | IN_PROGRESS | Persist durable Blob upload and seal | WSP-001, DATA-006 | regression repair is adding expiry reclamation before active-upload and reserved-byte quota admission, including temporary Blob cleanup, before the original durable seal and recovery evidence is accepted again |
...
| BRG-028 | IN_PROGRESS | Publish one locally authorized Artifact snapshot | WSP-001, ART-001, TASK-010, CON-010 | regression repair is moving byte capture behind an accepted lease, separating fresh attempts from ambiguous retries, and refreshing the active Bridge Workspace snapshot before the original publication and response-loss evidence is accepted again |
...
| QA-020 | IN_PROGRESS | Verify Artifact-to-Artifact two-Bridge recovery | BRG-028, BRG-029, ADP-014, TASK-011, WEB-040, QA-004, QA-006 | regression repair is extending the deterministic two-Bridge acceptance path with lease denial before file reads, live authority revocation, renewal after expiry, abandoned-upload reclamation, and root-level derived Artifact publication before the original recovery evidence is accepted again |
