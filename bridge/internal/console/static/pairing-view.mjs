export function pairingView(state, now = Date.now()) {
  ...
  const deadline = Date.parse(state.joinExpiresAt || "");
  const expired = active && (Boolean(enrollment.codeExpired) || (Number.isFinite(deadline) && deadline <= now));
  const seconds = Math.max(0, Math.ceil((deadline - now) / 1000));
  const canCopy = active && !expired && Boolean(state.joinCode) && Number.isFinite(deadline);
  const connection = state.connection?.state || "stopped";
  let guidance = "完成下方本机配置后即可申请审批码，无需打开终端。";
  ...
  return {
    status: active ? "等待新审批" : state.paired ? "已保存配对" : "尚未配对",
    guidance,
    ...
    codeText: canCopy ? state.joinCode : expired ? "审批码已过期" : "正在申请…",
    canCopy,
    expiry: canCopy
