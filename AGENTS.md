...
- `go run . -root ../../.. -mode generate` from `bridge/tools/windows-resources/` — regenerate the checked-in Windows PNG, multi-size ICO and amd64 resource object from the product SVG.
- `go test ./... && go vet ./... && go run . -root ../../.. -mode check` from `bridge/tools/windows-resources/` — verify the isolated icon tool and deterministic generated resources; format its Go source with `gofmt -w *.go`.
- `go run . -root ../../.. -mode verify -exe /absolute/path/to/ConveneWire-Bridge.exe` from `bridge/tools/windows-resources/` — inspect the actual Windows PE icon group and image resources rather than a generic Shell fallback.
